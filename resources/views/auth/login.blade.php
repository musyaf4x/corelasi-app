<x-guest-layout>
    <div class="w-full max-w-[420px] px-6 py-4 flex flex-col items-center mt-8 sm:mt-0">
        ...
        <div class="w-full bg-white border border-[#E2E8F0] shadow-sm rounded-xl p-6 mb-8">
            ...
            <!-- Error Banner -->
            @if ($errors->any())
                <div class="bg-[#FEF2F2] border border-[#FECACA] text-[#B91C1C] px-4 py-3 rounded-lg mb-6 flex items-start gap-3">
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="mb-5">
                    <div class="flex items-center justify-between mb-2">
                        <label for="password" class="block text-xs font-semibold text-[#64748B] tracking-wider uppercase">Kata Sandi</label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-xs font-semibold text-[#2E7D32] hover:underline">
                                Lupa Password?
                            </a>
                        @endif
                    </div>
                    ...
                    @if($errors->any())
                        <div class="mt-2 text-[#EF4444] text-xs flex items-start gap-1">
                            ...
                            <span>
                                ...
                                <span class="text-[#94A3B8]">Silahkan cek kembali atau <a href="{{ route('password.request') }}" class="text-[#2E7D32] hover:underline font-semibold">klik "Lupa Password"</a></span>
                            </span>
                        </div>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-end w-full">
                    <button type="submit" class="w-full bg-[#2A6D30] hover:bg-[#235827] text-white font-medium py-2.5 px-4 rounded-lg transition-colors flex items-center justify-center gap-2">
                        Masuk ke Sistem
            ...
            <div class="mt-8 bg-[#F8FAFC] rounded-lg p-4 flex items-start gap-3 border border-[#F1F5F9]">
                <svg class="w-5 h-5 text-[#2E7D32] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
