<x-guest-layout>
    <div class="w-full max-w-[420px] px-6 py-4 flex flex-col items-center mt-8 sm:mt-0">
        ...
        <div class="w-full bg-white border border-[#E2E8F0] shadow-sm rounded-xl p-6 mb-8">
            ...
            <!-- Error Banner -->
            @if ($errors->any())
                <div class="bg-[#FEF2F2] border border-[#FECACA] text-[#B91C1C] px-4 py-3 rounded-lg mb-6 flex items-start gap-3 relative">
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <!-- Email Address -->
                <div class="mb-5">
                    <label for="email" class="block text-xs font-semibold text-[#64748B] tracking-wider mb-2 uppercase">Email Institusi / Username</label>
                    <div class="relative">
                        ...
                        <input id="email" 
                               class="block w-full pl-10 pr-10 py-2.5 text-sm border @if($errors->any()) border-[#EF4444] text-[#EF4444] @else border-[#E2E8F0] focus:border-[#2E7D32] focus:ring focus:ring-[#2E7D32]/20 @endif rounded-lg outline-none transition-colors" 
                               type="text" 
                               name="email" 
                    ...
                    @if($errors->any())
                        <div class="mt-2 text-[#EF4444] text-xs flex items-start gap-1">
                            ...
                            <span><span class="font-semibold">Email tidak terdaftar.</span><br><span class="text-[#94A3B8]">Silahkan cek kembali atau hubungi admin</span></span>
                        </div>
                    @endif
                </div>
                ...
                <!-- Password -->
                <div class="mb-5">
                    <label for="password" class="block text-xs font-semibold text-[#64748B] tracking-wider mb-2 uppercase">Kata Sandi</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="block mb-6">
                    <label for="remember_me" class="inline-flex items-center cursor-pointer">
                        <input id="remember_me" type="checkbox" class="rounded border-[#CBD5E1] text-[#2E7D32] shadow-sm focus:ring-[#2E7D32] w-4 h-4" name="remember">
                        <span class="ms-2 text-sm text-[#64748B]">Tetap masuk di perangkat ini</span>
                    </label>
                </div>

                <div class="flex items-center justify-end w-full">
                    <button class="w-full bg-[#2A6D30] hover:bg-[#235827] text-white font-medium py-2.5 px-4 rounded-lg transition-colors flex items-center justify-center gap-2">
                        Masuk ke Sistem
            ...
            <div class="mt-8 bg-[#F8FAFC] rounded-lg p-4 flex items-start gap-3 border border-[#F1F5F9]">
                <svg class="w-5 h-5 text-[#2E7D32] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <p class="text-xs text-[#64748B] leading-relaxed">Akses untuk Admin, Guru, dan Siswa terdaftar. Jika mengalami kendala akun, hubungi admin sekolah.</p>
            </div>
        </div>
