<x-app-layout>
    ...
    <!-- Left Sidebar -->
    <aside
        class="w-64 bg-white border-r border-[#E2E8F0] min-h-screen flex flex-col fixed left-0 top-0 z-20 shadow-sm hidden md:flex">
        <!-- Logo Area -->
        <a href="{{ route('dashboard') }}"
            class="p-6 pb-8 border-b-0 border-[#E2E8F0] flex items-center gap-3 hover:opacity-80 transition-opacity">
        ...
        <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto no-scrollbar">
            ...
            @foreach($navItems as $item)
                @php
                    $isActive = false;
                    if (in_array($item['label'], ['Beranda', 'Administrasi Akademik']) && $currentRoute === $dashRoute) {
                        $isActive = true;
                    } elseif ($item['label'] === 'Materi dan Tugas' && str_starts_with($currentRoute, 'teacher.materi-tugas')) {
                        $isActive = true;
                    }
                    
                    $itemRoute = '#';
                    if (in_array($item['label'], ['Beranda', 'Administrasi Akademik'])) {
                        $itemRoute = route($dashRoute);
                    } elseif ($item['label'] === 'Materi dan Tugas') {
                        $itemRoute = route('teacher.materi-tugas');
                    }
                    
                    $activeClasses = $isActive 
                        ? 'bg-[#2D7336] text-white shadow-sm' 
                        : 'text-[#64748B] hover:bg-gray-50 hover:text-[#1E293B]';
                        
                    $iconClasses = $isActive ? 'text-white' : 'text-gray-400 group-hover:text-[#1E293B]';
                @endphp
                <a href="{{ $itemRoute }}"
                    class="group flex items-center gap-3 px-3 py-2.5 rounded-md font-medium text-sm transition-colors {{ $activeClasses }}">
                    @if($item['icon'] === 'home')
        ...
        <div class="px-4 py-4 border-t border-[#E2E8F0] space-y-1">
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-2">
                @csrf
                <button type="submit"
                    class="flex items-center gap-3 px-3 py-2.5 text-[#64748B] hover:bg-red-50 hover:text-red-600 rounded-md font-medium text-sm transition-colors w-full">
    ...
    <div class="md:ml-64 flex-1 flex flex-col min-h-screen relative w-full bg-[#F8FAFC]">
        ...
        <header
            class="h-16 border-b border-[#E2E8F0] bg-white/80 backdrop-blur sticky top-0 z-30 flex items-center justify-between px-4 md:px-8 w-full">
            <!-- Left: Semester Info -->
            <div>
                <button
                ...
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    <div class="flex items-center gap-3 cursor-pointer" @click="open = !open">
                        ...
                        <div class="w-9 h-9 rounded-full bg-gray-200 border border-gray-300 overflow-hidden">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=1E293B&color=fff"
        ...
        <main class="flex-1 p-4 md:p-8">
            ...
            @else
                <div class="w-full">
                    ...
                    <!-- Avatar Card -->
                    <div
                        class="bg-white rounded-xl shadow-[0_1px_3px_0_rgb(0,0,0,0.02)] border border-[#E2E8F0] p-6 md:p-8 mb-6 flex flex-col sm:flex-row items-center sm:items-start gap-6">
                        ...
                        <div class="flex-1 text-center sm:text-left mt-2 sm:mt-0">
                            ...
                            <span
                                class="inline-flex items-center gap-1.5 bg-[#2D7336] text-white text-sm font-semibold px-4 py-1.5 rounded-md mb-3">
                    ...
                    <!-- Data Diri Card -->
                    <div
                        class="bg-white rounded-xl shadow-[0_1px_3px_0_rgb(0,0,0,0.02)] border border-[#E2E8F0] p-6 md:p-8 mb-6">
                    ...
                    @if($role === 'admin')
                        <div
                            class="bg-white rounded-xl shadow-[0_1px_3px_0_rgb(0,0,0,0.02)] border border-[#E2E8F0] p-6 md:p-8">
                            <h2 class="flex items-center gap-2 text-xl font-bold text-[#1E293B] mb-5">
                                <svg class="w-5 h-5 text-[#1E293B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center bg-[#F8FAFC] rounded-md px-4 py-3">
                                    ...
                                    <div>
                                        <span
                                            class="inline-flex items-center bg-[#16A34A] text-white text-xs font-bold px-3 py-1 rounded">
                    ...
                    <!-- Keamanan Akun Card -->
                    <section
                        class="rounded-2xl border border-[#E2E8F0] bg-white p-6 md:p-8 shadow-[0_1px_3px_0_rgb(0,0,0,0.02)] mb-6">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div>
                                <h2 class="flex items-center gap-2 text-xl font-bold text-[#1E293B]">
                                    <svg class="w-5 h-5 text-[#1E293B]" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                    </svg>
